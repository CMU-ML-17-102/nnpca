...
#ifndef Opt_VectorUtils_h
#define Opt_VectorUtils_h

#include "Vector.h"
#include "Platform.h"

// Dense vector that does not own its data array
template<class BaseVector>
class VectorView {
public:
    VectorView(double *data, size_t size, size_t offset)
...
public:
    ConstVectorView(double *data, size_t size, size_t offset)
    : data_(data), size_(size), offset_(offset) {}
...
public:
    ...
    static void addVector(DenseVector &v,
        ...
        if(atomicComponentUpdates) {
            ...
            for(int i = 0; i < n; i++) {
                Platform::atomicAdd(raw + i, scale * increment[i]);
            }
        } else {
            int n = v.size();
            
            for(int i = 0; i < n; i++) {
                v[i] += scale * increment[i];
            }
        }
    }
    ...
    static inline void addCompatibleVec(
                                        Vector &self, double self_scale, const Vector &other, double other_scale) {
        int n = self.size();
